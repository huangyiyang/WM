...
const slicePathsAfterLastArrpath = (s) => {
  return s.slice(0, s.indexOf('.pam.'))
}

const transPathsAfterLastArrpath2String = compose(Join, Reverse, ToArray, slicePathsAfterLastArrpath, DelLastOne, ReplaceOne, Join, Reverse, ToArray);
...
function toSingelObject (json, path) {
  let paths = [];
  function execpath (_res, path) {
    /String/.test(datatype(path)) && (paths = path.split('.').slice(1));
    ...
    if (paths.length) {
      ...
      paths.length ? console.log(`next:${curpath}`, `next-paths:${paths}`) : console.log(`next:${curpath}`);
...
function toNameStringofMapValue (json, path) {
  let paths = [], mapItemPaths = [], NameStringofMapValue = '';
  return transPathsAfterLastArrpath2String(path)
}
export default {
  execpath2json,
  toSingelObject,
  toNameStringofMapValue
}
