<template>
  <div :class="['flowEidter', fullPage ? 'fullPage' : 'nomalArea']">
    <a-layout class="container">
      <a-layout-sider theme="light"
                      class="select-area flowEidter-sidebar">
        <a-row>
          <a-checkable-tag v-model="tag.checked0"
                           @change="toggleNodeShow0"
                           class="tag">工具</a-checkable-tag>
          <div align="center">
            <a-list :grid="{ gutter: 8, column: 1 }"
                    v-if="tag.toolShow">
              <a-list-item>
                <a-button-group>
                  <a-button v-for="(tool, index) in field.tools"
                            :key="index"
                            :icon="tool.icon"
        ...
        <a-row>
          <a-checkable-tag v-model="tag.checked1"
                           @change="toggleNodeShow1"
                           class="tag">基础节点</a-checkable-tag>
          <div align="center">
            <a-list :grid="{ gutter: 8, column: 2 }"
              ...
              <a-list-item v-for="(commonNode, index) in field.commonNodes"
                ...
                <div class="node-item"
                  ...
                  <template v-if="commonNode.icon !== 'GatewayIcon'">
        ...
        <a-row v-if="field.highNodes.length">
          <a-checkable-tag v-model="tag.checked2"
                           @change="toggleNodeShow2"
                           class="tag">高级节点</a-checkable-tag>
          <div align="center">
            <a-list :grid="{ gutter: 8, column: 2 }"
              ...
              <a-list-item v-for="(highNode, index) in field.highNodes"
                ...
                <div class="node-item"
                     :type="highNode.type"
                     belongto="highNodes">
                  <template v-if="highNode.icon !== 'GatewayIcon'">
      ...
      <a-layout>
        <a-layout-header class="header-option">
          ...
          <a-tooltip title="测试流程"
                     v-if="fullPage"
                     placement="bottom">
            <a-button @click="testFlow"
                      class="header-option-button"
                      size="small"
                      icon="robot"></a-button>
          </a-tooltip>
          ...
          <a-tooltip title="网页全屏"
                     placement="bottom">
            <a-button class="header-option-button"
                      @click="toggleEidterFullpage"
                      size="small"
                      :icon="fullPage ? 'fullscreen-exit' : 'fullscreen'"></a-button>
          </a-tooltip>
          <!-- </a-popconfirm> -->
          <!-- <a-tooltip title="退出"
                     placement="bottom">
        ...
        <a-layout-content class="content">
          ...
          <vue-context-menu :contextMenuData="linkContextMenuData"
                            @deleteLink="deleteLink">
          </vue-context-menu>
        </a-layout-content>
        <a-layout-footer class="foot">
          <span> {{ info.version }}, Powered by {{ info.author }}</span>
        </a-layout-footer>
      </a-layout>
      <a-layout-sider theme="light"
                      class="attr-area flowEidter-attrbar"
    ...
    <a-modal :title="'流程设计图_' + flowData.attr.id + '.png'"
             centered
             width="90%"
...
import { tools, Nodes } from './config/basic-node-config.js'
...
export default {
  ...
  props: ['interFaceFlowData', 'fullPage', 'nodesGroupName', 'files'],
  watch: {
    flowData: {
      handler (val) {
        this.syncFlowData(val)
      },
      deep: true
    }
  },
  ...
  computed: {
    field: function () {
      let { [this.nodesGroupName ? this.nodesGroupName : 'commonGroup']: {
        commonNodes,
        highNodes
      } } = Nodes;
      return {
  ...
  inject: ['processClearGetPathTargrt', 'syncFlowData', 'processSave', 'processTest', 'toggleEidterFullpage'],
  methods: {
    ...
    initJsPlumb () {
      ...
      that.plumb.bind('beforeDrop', function (info) {
        ...
        let filter1 = that.flowData.linkList.filter(link => (link.sourceId === sourceId && link.sourceId.indexOf('decision') > -1));//所有条件连线
        if (filter1.length > 1) {
          that.$message.error('条件节点连线只能有两条！');
          return false;
        }
        if (filter.length > 0) {
      ...
      that.plumb.bind('connection', function (conn, e) {
        ...
        if (that.flowData.status === flowConfig.flowStatus.CREATE || that.flowData.status === flowConfig.flowStatus.MODIFY) {
          ...
          label = '';
          // condition = 1;
          // let filter1 = that.flowData.linkList.filter(link => (link.sourceId === conn.sourceId && link.sourceId.indexOf('decision') > -1));
          // if (filter1.length > 0) {
          //   if (filter1[0].condition === '1') {
          //     label = 'NO';
          //     condition = '0';
          //   } else {
          //     label = 'YES';
        ...
        if (that.flowData.status !== flowConfig.flowStatus.LOADING) {
          // fatal bug
          let necessaryIndex = that.flowData.linkList.findIndex(link => (link.sourceId === conn.sourceId && link.targetId === conn.targetId));
          necessaryIndex === -1 && that.flowData.linkList.push(o);
        }
      });
    ...
    initFlow () {
      ...
      const that = this;
      if (this.interFaceFlowData.nodeList && this.interFaceFlowData.nodeList.length) {
        that.loadFlow(JSON.stringify(this.interFaceFlowData));
      } else {
    ...
    loadFlow (json) {
      ...
      that.plumb.batch(function () {
        ...
        let linkList = loadData.linkList.filter(link => (nodeIds.includes(link.sourceId) && nodeIds.includes(link.targetId)));
        that.$nextTick(() => {
          linkList.forEach(function (link, index) {
            let exits = that.flowData.linkList.filter(_link => {
              return link.sourceId === _link.sourceId && link.targetId === _link.targetId
            });

            // console.log(index, link, exits)
            //redundancy links delete
            // exits = [];
            if (!exits.length) {
    ...
    findNodeConfig (belongto, type, callback) {
      let node = null;
      let { commonNodes, highNodes, laneNodes } = this.field;
      switch (belongto) {
        case 'commonNodes':
    ...
    saveFlow () {
      ...
      console.log(JSON.stringify(flowObj, null, 2));
      // this.$message.success('保存流程成功！请查看控制台。');
      this.processSave(flowObj);
      return d;
    },
    exportFlowPicture () {
      const that = this;
    ...
    /*同步选择的path到对应节点属性数据*/
    forwardPathHook ({ targeNodeId, payload, targetAttr }) {
      console.log({ targeNodeId, payload, targetAttr })
      if (targeNodeId.includes('get')) {
        let upDateNode = this.flowData.nodeList.find(node => node.id === targeNodeId);
        upDateNode.newelementInfos = payload.table
        upDateNode.maxOrder = payload.maxOrder;
      } else if (targeNodeId.includes('wait')) {
        this.flowData.nodeList.find(node => node.id === targeNodeId).waitUntilEle = payload;
      } else if (['captcha', 'custom', 'download', 'smsVerification', 'chooseFile'].filter(l => targeNodeId.includes(l)).length) {
        this.flowData.nodeList.find(node => node.id === targeNodeId)[targetAttr] = payload;
      } else {
        this.flowData.nodeList.find(node => node.id === targeNodeId).elementPath = payload;
        if (targeNodeId.includes('checkLogin')) {
          window._wmGetSelectWndUrl && window._wmGetSelectWndUrl(window._wmGetSelectWndUrlCallback)
        }
      }
    ...
    reFreshEditor () {
      let that = this;
      let dur = 300;
      if (this.freshing || !this.flowData.nodeList.length) {
        return false;
      }
      this.freshing = true;
      this.freshCache = JSON.stringify(this.flowData);
      setTimeout(() => {
        that.clear();
        setTimeout(() => {
          that.loadFlow(that.freshCache);
          setTimeout(() => {
            that.freshCache = '';
            that.freshing = false;
            that.selectTool('drag');
