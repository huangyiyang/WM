<template>
  <div style="width: 100%; height: 100%; overflow: hidden; position: relative;">
    ...
    <div id="flowContainer"
      ...
      <flow-node v-for="(node, index) in flowData.nodeList"
                 :key="index"
                 :node="node"
                 :files="files"
                 :plumb="plumb"
    ...
    <vue-context-menu :contextMenuData="containerContextMenuData"
                      @flowInfo="flowInfo"
                      @paste="paste"
                      @selectAll="selectAll"
                      @saveFlow="saveFlow"
    ...
    <vue-context-menu :contextMenuData="nodeContextMenuData"
                      @copyNode="copyNode"
...
export default {
  props: ['browserType', 'flowData', 'plumb', 'select', 'selectGroup', 'currentTool', 'files'],
  components: {
    jsplumb,
    FlowNode
  },
  ...
  methods: {
    ...
    paste () {
      ...
      that.clipboard.forEach(function (node, index) {
        ...
        //深拷贝数据持续优化
        if (newNode.elementInfos) {
          newNode.elementInfos = JSON.parse(JSON.stringify(node.elementInfos));
        }
