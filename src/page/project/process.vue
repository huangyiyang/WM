  ...
  <div class="management-list-except project-interface"
    ...
    <section class="project-module">
      <p class="project-module-name">
        数据接口管理 <span>[{{ detailData.processName || '新增数据接口' }}]</span>
      </p>
      <div class="rt">
        <template v-if="status.isPublished === 1">
          ...
          <el-button type="primary"
                     @click="readApi">发布接口说明</el-button>
    ...
    <el-dialog top="0"
      ...
      <div class="fill-all flex-column">
...
export default {
  ...
  created () {
    ...
    this.detailData.type = this.$route.params.processType
  },
  watch: {
    '$route' (to, form) {
      this.isSelect = '';
  ...
  methods: {
    ...
    interfaceDetail () {
      if (this.$route.query.id) {
        ...
        this.$axios.post('/process/detail', data, s => {
          this.detailData.checkLogin = s.data.checkLogin
          this.detailData.checkTimes = s.data.checkTimes
    ...
    interfaceSave (fun) {
      ...
      let filter1 = this.detailData.flowData.linkList.filter(link => (link.sourceId.indexOf('decision') > -1));
      // if (filter1.length > 0) {
      //   if (filter1[0].condition !== 1 || filter1[0].condition !== 0) {
      //     this.$message.error('条件节点连线属性未选择！');
      //     return false;
      //   }
      // }
      data.append('flowData', JSON.stringify(this.detailData.flowData));
      data.append('sendData', JSON.stringify(ZFSN.filterSendJson(this.detailData.flowData)));
      //if (this.canSave) {
      this.canSave = false;
      this.$axios.post('/process/save', data, s => {
        typeof fun === 'object' ? this.$message.success(s.message) : fun();
        this.$emit('update-list', true);
        this.$router.push({ path: '/project/process', query: { id: s.id } });
        this.interfaceDetail();
        this.canSave = true;
      }, f => {
        this.canSave = true;
    ...
    iFrameLoad (fun) {
      if (window._wmSelectElementPath) {
        ...
        let sessionid = that.detailData.autoLoginId ? that.detailData.autoLoginId : '';
        let capturetype = 0;
        //that.pageVisible && window._wmSelectElementPath(that.snapShotState.targetUrl, !that.snapShotState.loaded, type, that.selectClickHandleForClent);
    ...
    * @param {Object} item —— 当前点击多选框对应的路径对象 */
    ...
    selectClickHandleForClent (path) {
      ...
      console.log(path);
      //获取截屏文件流
      console.log(this.base64ConvertFile(this.arrayBufferToBase64(window._wmGetCaptureImgArrayBuffer(0)), 'bigimg'))
      ...
      let file = this.base64ConvertFile(this.arrayBufferToBase64(window._wmGetCaptureImgArrayBuffer(0)), 'bigimg')
      this.uploadFile(nodeId, type, processId, file)
      if (this.nodeid.indexOf('click') > -1 || this.nodeid.indexOf('get') > -1) { //type 3 是除登录外的流程大图
        type = 1;
