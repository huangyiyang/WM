  ...
  <div class="management-list-except project-interface"
    ...
    <section class="project-module">
      <p class="project-module-name">
        数据接口管理 <span>[{{ detailData.interfaceName || '新增提交接口' }}]</span>
      </p>
      <div class="rt">
    ...
    <section class="project-info">
      <el-scrollbar>
        ...
        <div class="project-list">
          <div class="project-list-row">
            ...
            <div class="quarter">
              ...
              <el-select v-model="detailData.autoLoginId"
                         placeholder="请选择"
                         @change="iFrameLoad">
          ...
          <div class="project-list-row">
            <div class="half longsite accessTarget">
              ...
              <el-input v-model="detailData.targetUrl"
                        @blur="accessValidate(snapShotchange)"></el-input>
              <i v-show="!loginLoading"
                 class="el-icon-loading"></i>
              <i v-show="loginLoading"
                 class="el-icon-check"></i>
              <div :class="['accessinfo', {'fail': accessFail}]"
                   v-if="accessInfo">
        ...
        <div class="project-list-row">
          <div class="quarter">
            <label for="">提交：</label>
            <el-input :class="{activfe: isSelect === 'submit'}"
                      v-model="detailData.commitValue.elementPath"></el-input>
            <i title=""
               @click="getPath('submit')"
               :class="['el-icon-thumb', {active: isSelect === 'submit'}]"></i>
          </div>
        </div>
        <p class="project-title">表单参数配置</p>
            ...
            <el-table-column min-width="190"
              ...
              <template slot-scope="scope">
                <template v-if="scope.row.isCustom === 0">
                  ...
                  <span title=""
                        @click="getPath(scope.$index, scope.row)"
...
export default {
  ...
  methods: {
        ...
        this.$axios.post('/interface/detail', data, s => {
          for (let key in this.detailData) {
            if (typeof this.detailData[key] !== 'object') {
              ...
              this.detailData[key].elementPath = s.data[key].elementPath;
              this.detailData[key].elementPathType = s.data[key].elementPathType;
            }
          }
    ...
    interfaceSelect () {
      let data = new URLSearchParams();
      data.append('type', 0)
      this.$axios.post('/interface/selectDataByType', data, s => {
    ...
    interfaceSave (fun) {
      ...
      if (this.canSave) {
        ...
        this.$axios.post('/interface/save', data, s => {
          ...
          this.$emit('update-list', true);
          this.$router.push({ path: '/project/interfaceSubmit', query: { id: s.id } });
          //this.$router.push({path: '/developer/interface'});
          this.canSave = true;
        }, f => {
          this.canSave = true;
        });
    ...
    * @param {Object} item —— 当前点击多选框对应的路径对象 */
    selectClick (val, item) {
      ...
      if (val) {
        if (this.isSelect === 'submit') {
          //this.tableData[this.isSelect].elementPath = item.sPath;
          this.detailData.commitValue.elementPath = item.sPath
        }
